...
def search(customer):
	...
	def find_info(iterator, plus: bool):
		...
		"""
		price = iterator.find("span", class_="RestaurantDeliveryInfo__price").text.strip()
		tiime = iterator.find("span", class_="RestaurantDeliveryInfo__time").text.strip()
		try:
			count = iterator.find("span", class_="RestaurantRating__rating-count").text.strip()
			if count == "Nehodnocena":
				rating = "?"
			else:
				rating = iterator.find("span", class_="RestaurantRating__info").text.strip()
	...
	for restaurant in restaurace.find_all("a", class_="Restaurant Restaurant-box Restaurant--djPlus"):
